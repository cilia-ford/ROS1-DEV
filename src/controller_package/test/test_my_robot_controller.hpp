...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_ROBOT_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_ROBOT_CONTROLLER_HPP_
...
#include "controller_package/my_robot_controller.hpp"
...
using ControllerStateMsg = controller_package::.::ControllerStateMsg;
using ControllerReferenceMsg = controller_package::.::ControllerReferenceMsg;
using ControllerModeSrvType = controller_package::.::ControllerModeSrvType;
...
class Testable. : public controller_package::.
{
  FRIEND_TEST(.Test, all_parameters_set_configure_success);
  FRIEND_TEST(.Test, activate_success);
  FRIEND_TEST(.Test, reactivate_success);
  FRIEND_TEST(.Test, test_setting_slow_mode_service);
  FRIEND_TEST(.Test, test_update_logic_fast);
  FRIEND_TEST(.Test, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = controller_package::.::on_configure(previous_state);
...
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_my_robot_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_my_robot_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_my_robot_controller")
  {
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_my_robot_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<Testable.> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_ROBOT_CONTROLLER_HPP_
